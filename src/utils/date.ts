 ...
 */
const convertISOToLocal = (
  isoString: string,
  onlyDate?: boolean,
  useUSACalendar?: boolean
): string => {
  if (checkFalsy(isoString)) return "";
  ...
  if (useUSACalendar) {
    return format(date, onlyDate ? "MM/dd/yyyy" : "MM/dd/yyyy hh:mm a");
  }

  return format(date, onlyDate ? "yyyy/MM/dd" : "yyyy/MM/dd hh:mm a"); // 로컬 타임존 기준으로 포맷
};
